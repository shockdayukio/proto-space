...
function appendSubImageForm(next_sub_image_id){
  var html = '<li class="sub-image-item col-md-4">'+
               '<div class="image-upload">' +
                 '<img>' +
                 '<input type="file" name="prototype[captured_images_attributes][' + next_sub_image_id + '][image]" id="prototype_captured_images_attributes_' + next_sub_image_id + '_image" class="js-image-file js-image">' +
                 '<input type="hidden" name="prototype[captured_images_attributes][' + next_sub_image_id + '][status]" id="prototype_captured_images_attributes_' + next_sub_image_id + '_status" value="sub">' +
               '</div>' +
             '</li>';
...
$(document).on('click', '#js-image-plus', function(){
  ...
  if (sub_image_item_count <= MAXIMUM_NUMBER_OF_SUB_IMAGES_IN_PROTOTYPE){
    var next_sub_image_id = $(".js-image-file").length;
    appendSubImageForm(next_sub_image_id);
    if (sub_image_item_count < MAXIMUM_NUMBER_OF_SUB_IMAGES_IN_PROTOTYPE){
